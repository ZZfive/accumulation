...
class Executor:
    ...
    def execute(self, question: str, plan: List[str], **kwargs) -> str:
        ...
        for i, step in enumerate(plan, 1):
            ...
            prompt = self.prompt_template.format(question=question, plan=plan,
            ...
            print(f"✅ 步骤 {i} 完成，结果: {final_answer}")
        
        return final_answer


class PlanAndSloveAgent(Agent):
    r"""
    ...
    def __init__(self, name: str, llm: LLM, system_prompt: str = None, config: Config = None,
        ...
        Args:
            ...
            system_prompt: 系统提示词
            config: 配置对象
            custom_prompt: 自定义提示词字典 {"planner": "规划器提示词", "executor": "执行器提示词"}
        ...
        if custom_prompts:
            planner_prompt, executor_prompt = custom_prompts.get("planner"), custom_prompts.get("executor")
        else:
            planner_prompt, executor_prompt = None, None
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        Returns:
            最终答案
        """
        print(f"\n🤖 {self.name} 开始解决复杂问题: {input_text}")

        # 1. 生成执行计划
        plan = self.planner.plan(input_text, **kwargs)
        if not plan:
            final_answer = "无法生成有效的执行计划，任务终止"
            print(f"\n--- 任务终止 ---\n{final_answer}")
...
if __name__ == "__main__":
    ...
    llm = LLM()
    agent = PlanAndSloveAgent(name="规划执行助手", llm=llm)

...
说明：
- 提供统一的文本嵌入接口与多实现：本地Transformer、DashScope、TF-IDF兜底
- 暴露 get_text_embedding()/get_dimension()/refresh_embedder() 接口供各记忆类型统一使用
...
class LocalTransformerEmbedding(EmbeddingModel):
    r"""本地Transformer嵌入：优先sentence-transformers，缺失回退transformer+torch"""
    ...
    def _load_backend(self):
        ...
        raise ImportError("Failed to load embedding backend: sentence-transformers or transformers+torch not found")

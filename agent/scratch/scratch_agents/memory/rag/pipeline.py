...
def _is_cjk(ch: str) -> bool:  # 判断是否为中日韩字符
    code = ord(ch)
    return (
...
def _approx_token_len(text: str) -> int:  # 近似估计文本的token长度
...
def _split_paragraphs_with_headings(text: str) -> List[Dict]:  # 分割文本为段落，并提取标题
    lines = text.splitlines()
...
def _chunk_paragraphs(paragraphs: List[Dict], chunk_tokens: int, overlap_tokens: int) -> List[Dict]:  # 分割段落为chunks
    chunks: List[Dict] = []
    cur: List[Dict] = []
    cur_tokens = 0
    i = 0
...
def load_and_chunk_texts(paths: List[str], chunk_size: int = 800, chunk_overlap: int = 100,
                         namespace: Optional[str] = None, source_label: str = "rag") -> List[Dict]:  # 加载并分割文本
    """
...
def compute_graph_signals_from_pool(vector_hits: List[Dict], same_doc_weight: float = 1.0, proximity_weight: float = 1.0,
                                    proximity_window_chars: int = 1600) -> Dict[str, float]:  # 计算图信号
    """

...
class PerceptualMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        base_collection = os.getenv("QDRANT_COLLECTION", "agents_vectors")
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        try:
            ...
            store = self._get_vector_store_for_modality(modality)  # 获取对应的向量存储
            store.add_vectors(
                vectors=[vector],
                metadata=[{
                    "memory_id": memory_item.id,
    ...
    class _no_grad:  # 自定义的上下文管理器，用于禁用梯度计算
        def __enter__(self):
            try:
                import torch
                self.prev = torch.is_grad_enabled()  # 进入上下文管理器时，记录当前梯度计算状态
        ...
        def __exit__(self, exc_type, exc, tb):
            try:
                ...
                if self.prev is not None:
                    torch.set_grad_enabled(self.prev)  # 退出上下文管理器时，恢复梯度计算状态
            except Exception:
                pass
